...
import { CatalogItem, Sale, SaleRequest } from '../../core/models/api.models';
import { CatalogService, CatalogType } from '../../core/services/catalog.service';
...
export class SaleFormComponent implements OnInit {
  private readonly fb = inject(FormBuilder); private readonly service = inject(SalesService); private readonly catalog = inject(CatalogService); private readonly errorService = inject(ErrorService); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router);
  readonly loading = signal(false); readonly message = signal(''); readonly editMode = this.route.snapshot.paramMap.has('id'); readonly customers = signal<CatalogItem[]>([]); readonly branches = signal<CatalogItem[]>([]); readonly products = signal<Record<number, CatalogItem[]>>({}); private id = '';
  ...
  searchCatalog(type: CatalogType, search: string, index?: number): void { this.catalog.search(type, search).subscribe({ next: ({ data }) => { if (type === 'customers') this.customers.set(data); else if (type === 'branches') this.branches.set(data); else if (index !== undefined) this.products.update(current => ({ ...current, [index]: data })); }, error: () => { if (type === 'customers') this.customers.set([]); else if (type === 'branches') this.branches.set([]); else if (index !== undefined) this.products.update(current => ({ ...current, [index]: [] })); } }); }
  selectCustomer(item: CatalogItem): void { this.form.patchValue({ customerId: item.id, customerName: item.name }); this.customers.set([]); }
  selectBranch(item: CatalogItem): void { this.form.patchValue({ branchId: item.id, branchName: item.name }); this.branches.set([]); }
  selectProduct(index: number, item: CatalogItem): void { this.items.at(index).patchValue({ productId: item.id, productName: item.name }); this.products.update(current => ({ ...current, [index]: [] })); }
  customerInput(search: string): void { this.form.controls.customerId.setValue(''); this.searchCatalog('customers', search); }
  branchInput(search: string): void { this.form.controls.branchId.setValue(''); this.searchCatalog('branches', search); }
  productInput(index: number, search: string): void { this.items.at(index).get('productId')?.setValue(''); this.searchCatalog('products', search, index); }
